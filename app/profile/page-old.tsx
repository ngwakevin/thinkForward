...
export default async function ProfilePage() {
  ...
  try {
    ...
    if (user) initial = { ...(user.profile || {}), ...user };
  } catch (_) {
  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-10 space-y-10">
      <Header initial={initial} fallbackName={session.user?.name || session.user?.email || 'Your Profile'} />
      <AccountTabs user={session.user as any} initialData={initial} />
    </main>
  );
}

function Header({ initial, fallbackName }: { initial: any; fallbackName: string }) {
