...
interface ProductsGridProps { initialProducts: ProductDefinition[]; formatSelectorPosition?: 'top' | 'bottom' | 'none'; }
...
export default function ProductsGrid({ initialProducts, formatSelectorPosition = 'top' }: ProductsGridProps) {
  ...
  // Large format selector buttons (Live Bootcamps -> /bootcamps, Self-Paced -> /courses)
  ...
  const showTopSelector = formatSelectorPosition === 'top';
  ...
  {/* Filters: simplified labels for easier navigation */}
      ...
      placeholder="Search courses"
        ...
        <div className="flex items-center justify-between text-[11px] text-fg-muted">
          <span>Showing <span className="text-fg font-medium">{shown.length}</span> track{shown.length===1?'':'s'}</span>
          {message && <div className="text-accent">{message}</div>}
        </div>
      </div>
  <div id="courses-list" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 scroll-mt-32">
      ...
      {showBottomSelector && showSelector && (
        <div className="space-y-4">
          <div className="pt-4 border-t border-border/60">
            <h3 className="font-display text-lg font-semibold tracking-tight text-fg">Explore formats</h3>
            <p className="text-sm text-fg-muted">Choose between cohort-based live bootcamps and flexible self-paced learning.</p>
          </div>
          <FormatSelector />
        </div>
      )}
