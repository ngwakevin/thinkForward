	...
	return (
		<section ref={ref} className="bg-bg-alt/40 py-12 border-y border-border/60">
			<div className="mx-auto max-w-6xl px-6 grid gap-8 sm:grid-cols-2 md:grid-cols-4 text-center">
				{stats.map(s => {
					const val = useCountUp(s.value, 1200, active && !prefersReduced);
					return (
						<div key={s.label} className="transition-opacity" style={{ opacity: active ? 1 : 0.4 }}>
							...
							<div className="mt-2 text-[11px] uppercase tracking-[0.15em] text-fg-muted">{s.label}</div>
						</div>
					);
				})}
			</div>
