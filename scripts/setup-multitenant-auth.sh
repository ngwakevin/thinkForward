...
# Script to manually configure Azure Web App with multi-tenant authentication settings
...
echo -e "${BLUE}=================================================${NC}"
echo -e "${BLUE}     Configure Multi-Tenant Authentication        ${NC}"
echo -e "${BLUE}=================================================${NC}"
...
echo -e "${GREEN}Setting multi-tenant authentication for ${WEBAPP_NAME}...${NC}"
...
# Update auth settings in the web app
...
if [ $? -eq 0 ]; then
  echo -e "${GREEN}Multi-tenant authentication settings updated successfully for ${WEBAPP_NAME}!${NC}"
  echo -e "${GREEN}Restarting the web app to apply changes...${NC}"
  
  # Restart the web app to ensure changes take effect
  az webapp restart --resource-group "$RESOURCE_GROUP" --name "$WEBAPP_NAME"
  
  echo -e "${GREEN}Done! The application has been updated with multi-tenant authentication settings.${NC}"
  ...
  echo -e "${YELLOW}Important: Make sure this callback URL is registered in your Azure AD app registration,${NC}"
  echo -e "${YELLOW}which is already correctly configured for multi-tenant authentication.${NC}"
else
  echo -e "${RED}Failed to update authentication settings. Please check your credentials and try again.${NC}"
fi